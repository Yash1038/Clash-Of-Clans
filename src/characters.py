    ...
    def move(self, pos):
        ...
        if(r + c == 1):
            ...
            if(pos[0] > self.position[0]):
                    ...
                    self.position[0] -= 1
        self.heal_troop(pos[0], pos[1])

    def heal_troop(self, x, y):
        if(abs(x - self.position[0]) + abs(y - self.position[1]) > self.heal_range):
            return
        troops = [barbarians, archers, dragons, balloons, stealtharchers]
        for troop in troops:
            for t in troop:
                if(abs(t.position[0] - x) <= self.heal_radius and abs(t.position[1] - y) <= self.heal_radius):
...
def spawnStealthArcher(pos):
    ...
    # stealtharcher.currentTime = time.time()
...
def move_healers(V):
    for hl in healers:
        if(hl.alive == False):
            continue
        closest_troop = search_for_closest_troop(hl.position)
...
def search_for_closest_troop(pos):
    ...
    for troop in troops:
        ...
        if(dist < closest_dist and dist != 0 and troop.health < troop.max_health):
            flag = 1
            closest_dist = dist
