...
def starchColor(starch):
    health = starch.health
    max_health = starch.max_health
    currTime = starch.currentTime
    percentage = (health*100)/max_health
...
def printMap(V):
    ...
    for starch in stealtharchers:
        a= 2*starch.position[0]
        b = 2*starch.position[1]
        map_matrix[a][b] = starchColor(starch) + Fore.BLACK + 'ST'
        map_matrix[a+1][b] = starchColor(starch) + Fore.BLACK + '  '
        map_matrix[a][b+1] = starchColor(starch) + Fore.BLACK + 'AR'
        map_matrix[a+1][b+1] = starchColor(starch) + Fore.BLACK + 'CH'
    ...
    for hl in healers:
        ...
        map_matrix[a+1][b] = hlColor(hl) + Fore.BLACK + ' '
