...
class Village:
    ...
    def generate_map(self):
        ...
        for wall in self.walls_top:                                     # wall is 1x1
            wall_obj = bd.Wall(wall, self, rd.randint(1, 5))
        ...
        for cannon in self.cannons:                                     # cannon is 2x2
            cannon_obj = bd.Cannon(cannon, self, rd.randint(1, 5))
        ...
        for wizard_tower in self.wizard_towers:                          # wizard tower is 1X1
            wizard_tower_obj = bd.WizardTower(wizard_tower, self, rd.randint(1, 5))
    ...
    def check_if_game_over(self, King):
        ...
        if len(all_buildings) == 0 and self.town_hall_obj == None:
            if self.level != 3:
                ...
                return 2
        elif King.alive == False and len(barbarians) == 0 and len(dragons) == 0 and len(balloons) == 0 and len(archers) == 0 and len(stealtharchers):
            return 3
        else:
            return 0
