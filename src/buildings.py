...
class Cannon(Building):
    def __init__(self, position, V, cannon_level):
        self.position = position
        self.dimensions = (2, 2)
        self.V = V
        self.cannon_level = cannon_level
        self.destroyed = False
        self.health = 60 + 30*(self.cannon_level)
        self.max_health = 60 + 30*(self.cannon_level)
        self.type = 'cannon'
        self.attack = 4 + (self.cannon_level)
        self.attack_radius = 5 + (self.cannon_level)/2
    ...
    def scan_for_targets(self, King):
        ...
        for sta in stealtharchers:
            if (sta.position[0] - self.position[0])**2 + (sta.position[1] - self.position[1])**2 <= self.attack_radius**2:
                if (time.time() - sta.currentTime <= 10):
                    return
                self.isShooting = True
                self.attack_target(sta)
                return
...
class Wall(Building):
    def __init__(self, position, V, wall_level):
        self.position = position
        self.dimensions = (1, 1)
        self.V = V
    ...
    def blast(self):
        if(self.wall_level <3):
            return
        
        troops = barbarians + archers + stealtharchers
        for troop in troops:
            if (troop.position[0] - self.position[0]) < 2 and (troop.position[1] - self.position[1]) < 2:
...
class WizardTower(Building):
    def __init__(self, position, V, wizard_tower_level):
        self.position = position
        self.dimensions = (1, 1)
        self.V = V
        self.wizard_tower_level = wizard_tower_level
        self.destroyed = False
        self.health = 60 + 30*(self.wizard_tower_level)
        self.max_health = 60 + 30*(self.wizard_tower_level)
        self.type = 'wizardtower'
        self.attack = 4 + (self.wizard_tower_level)
        self.attack_radius = 5 + (self.wizard_tower_level)/2
        self.isShooting = False
